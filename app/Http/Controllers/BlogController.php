...
class BlogController extends Controller
{
    ...
    public function TagAdd($id)
    {
        $categories = Category::all();
        $blog  = Blog::with('tags')->find($id);
        return view('admin.blogs.tag_add',compact('blog','categories'));
    }
    public function TagCreate(StoreTagRequest $request,$id)
    {
       $request = $request->validated();
        $blog = Blog::with('tags')->find($id);
        $blog->tags()->delete();
        ...
        return redirect()->route('blog.index')->with('success','Tag Created');
    }
